...
首先需要在ArkTS 中定义XComponent并设置 XComponentController。XComponent组件用于在UI中嵌入基于OpenGL或Vulkan等图形API实现的渲染内容。
Column() {
XComponent({
id: 'myXComponent',
...
创建 XComponentController子类，实现回调方法：
...
使用surfaceId获取NativeWindow：surfaceId是在XComponent创建过程中生成的。在onSurfaceCreated 回调中，可以使用OH_NativeWindow_CreateNativeWindowFromSurfaceId函数通过surfaceId获取nativeWindow。
...
有关ArkTS XComponent 组件的使用，请参考：ArkTS XComponent组件使用示例。
