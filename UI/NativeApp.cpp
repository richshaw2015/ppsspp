...
void NativeInit(int argc, const char *argv[], const char *savegame_dir, const char *external_dir, const char *cache_dir) {
	...
	g_Config.flash0Directory = Path(savegame_dir) / "flash0";
	
	// 注册资源目录 - OHOS 的 rawfile 资源会被复制到 filesDir/assets
	Path ohosAssetsPath = Path(savegame_dir) / "assets";
	INFO_LOG(Log::System, "OHOS: Registering assets from %s", ohosAssetsPath.c_str());
	
	if (File::Exists(ohosAssetsPath)) {
		g_VFS.Register("", new DirectoryReader(ohosAssetsPath));
		INFO_LOG(Log::System, "OHOS: Assets registered successfully");
	} else {
	...
	// 也尝试从 cache 目录读取（备用）
	if (cache_dir && strlen(cache_dir)) {
		Path cacheAssetsPath = Path(cache_dir) / "assets";
		if (File::Exists(cacheAssetsPath)) {
			g_VFS.Register("", new DirectoryReader(cacheAssetsPath));
			INFO_LOG(Log::System, "OHOS: ✓ Registered assets from cache %s", cacheAssetsPath.c_str());
		}
	}
	...
	INFO_LOG(Log::System, "OHOS: memStickDirectory = %s", g_Config.memStickDirectory.c_str());
	INFO_LOG(Log::System, "OHOS: flash0Directory = %s", g_Config.flash0Directory.c_str());
	...
	// Stdio is used for Android/OHOS logging too.
